...
func (req *Request)Host(host string)*Request{
	req.host=host
	return req
}
...
func (req *Request) Do() (res Response, err error) {
	...
	httptrans := &http.Transport{
		...
		//		return nil, err
		//	}
		//	conn.SetDeadline(time.Now().Add(time.Second * 1))    //设置发送接受数据超时
		//	return conn, nil
		//},
	...
	httpclient.Transport=httptrans

	if httpres, err = httpclient.Do(httpreq);
	err != nil {
		return
	}
